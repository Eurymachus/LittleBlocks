...
public class BlockPistonBase extends Block {
	...
	public int getBlockTextureFromSideAndMetadata(int par1, int par2) {
		...
		return var3 > 5 ? this.blockIndexInTexture
				: (par1 == var3 ? (!isExtended(par2) && this.minX <= 0.0D
						&& this.minY <= 0.0D && this.minZ <= 0.0D
						&& this.maxX >= 1.0D && this.maxY >= 1.0D
						&& this.maxZ >= 1.0D ? this.blockIndexInTexture : 110)
	...
	private void updatePistonState(World par1World, int par2, int par3, int par4) {
		...
		boolean var7 = this.isIndirectlyPowered(par1World, par2, par3, par4,
				var6);
		...
		if (var5 != 7) {
			if (var7 && !isExtended(var5)) {
				if (canExtend(par1World, par2, par3, par4, var6)) {
					...
					par1World.sendClientEvent(par2, par3, par4, 0, var6);
				}
			} else if (!var7 && isExtended(var5)) {
				par1World.setBlockMetadata(par2, par3, par4, var6);
				par1World.sendClientEvent(par2, par3, par4, 1, var6);
			}
		}
	}
	...
	private boolean isIndirectlyPowered(World par1World, int par2, int par3,
		...
		return par5 != 0
				...
				&& par1World.isBlockIndirectlyProvidingPowerTo(par2, par3,
						par4 - 1, 2) ? true : (par5 != 3
				&& par1World.isBlockIndirectlyProvidingPowerTo(par2, par3,
						par4 + 1, 3) ? true : (par5 != 5
				&& par1World.isBlockIndirectlyProvidingPowerTo(par2 + 1, par3,
						par4, 5) ? true : (par5 != 4
				&& par1World.isBlockIndirectlyProvidingPowerTo(par2 - 1, par3,
						par4, 4) ? true : (par1World
				.isBlockIndirectlyProvidingPowerTo(par2, par3, par4, 0) ? true
				: (par1World.isBlockIndirectlyProvidingPowerTo(par2, par3 + 2,
						par4, 1) ? true : (par1World
						.isBlockIndirectlyProvidingPowerTo(par2, par3 + 1,
								par4 - 1, 2) ? true : (par1World
						.isBlockIndirectlyProvidingPowerTo(par2, par3 + 1,
								par4 + 1, 3) ? true : (par1World
						.isBlockIndirectlyProvidingPowerTo(par2 - 1, par3 + 1,
								par4, 4) ? true : par1World
						.isBlockIndirectlyProvidingPowerTo(par2 + 1, par3 + 1,
								par4, 5)))))))))));
	}
	...
	/**
	 * Called when the block receives a client event - see
	 * World.sendClientEvent. By default, passes it on to the tile entity at
	 * this location. Args: world, x, y, z, event number, parameter
	 */
	public void receiveClientEvent(World par1World, int par2, int par3,
		...
		if (par5 == 0) {
			if (this.tryExtend(par1World, par2, par3, par4, par6)) {
				par1World
						.setBlockMetadataWithNotify(par2, par3, par4, par6 | 8);
			...
			par1World.setBlockAndMetadata(par2, par3, par4,
					Block.pistonMoving.blockID, par6);
			par1World.setBlockTileEntity(par2, par3, par4, BlockPistonMoving
					.getTileEntity(this.blockID, par6, par6, false, true));
			...
			if (this.isSticky) {
				...
				if (!var14
						&& var12 > 0
						&& canPushBlock(var12, par1World, var9, var10, var11,
								false)
						&& (Block.blocksList[var12].getMobilityFlag() == 0
								|| var12 == Block.pistonBase.blockID || var12 == Block.pistonStickyBase.blockID)) {
					par2 += Facing.offsetsXForSide[par6];
					par3 += Facing.offsetsYForSide[par6];
					par4 += Facing.offsetsZForSide[par6];
					par1World.setBlockAndMetadata(par2, par3, par4,
							Block.pistonMoving.blockID, var13);
					par1World.setBlockTileEntity(par2, par3, par4,
							BlockPistonMoving.getTileEntity(var12, var13, par6,
									false, false));
					ignoreUpdates = false;
	...
	private static int determineOrientation(World par0World, int par1,
		...
		// SlimeVoid START
		if (par0World instanceof LittleWorld) {
			par1 >>= 3;
			par2 >>= 3;
			par3 >>= 3;
		}
		...
		if (MathHelper.abs((float) par4EntityPlayer.posX - par1) < 2.0F
			...
			double var5 = par4EntityPlayer.posY + 1.82D
					- par4EntityPlayer.yOffset;
		...
		int var7 = MathHelper
				.floor_double((par4EntityPlayer.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3;
		return var7 == 0 ? 2 : (var7 == 1 ? 5 : (var7 == 2 ? 3 : (var7 == 3 ? 4
				: 0)));
	}
	...
	private static boolean canPushBlock(int par0, World par1World, int par2,
		...
		if (par0 == Block.obsidian.blockID) {
			...
			return !(Block.blocksList[par0] != null && Block.blocksList[par0]
					.hasTileEntity(par1World.getBlockMetadata(par2, par3, par4)));
		}
	}
	...
	private boolean tryExtend(World par1World, int par2, int par3, int par4,
		...
		while (true) {
			...
			if (var9 < 13) {
				// SlimeVoid START
				if (var6 <= 0 || var6 >= par1World.getHeight() - 1)
				// SlimeVoid END
				// if (var6 <= 0 || var6 >= 255) OLD CODE
				{
					return false;
				...
				if (var10 != 0) {
					...
					Block.blocksList[var10].dropBlockAsItem(par1World, var6,
							var7, var8,
			...
			while (var6 != par2 || var7 != par3 || var8 != par4) {
				...
				if (var12 == this.blockID && var9 == par2 && var10 == par3
						&& var11 == par4) {
					par1World.setBlockAndMetadata(var6, var7, var8,
							Block.pistonMoving.blockID, par5
									| (this.isSticky ? 8 : 0));
					par1World.setBlockTileEntity(var6, var7, var8,
							BlockPistonMoving.getTileEntity(
									Block.pistonExtension.blockID, par5
											| (this.isSticky ? 8 : 0), par5,
									true, false));
				} else {
					par1World.setBlockAndMetadata(var6, var7, var8,
							Block.pistonMoving.blockID, var13);
					par1World.setBlockTileEntity(var6, var7, var8,
							BlockPistonMoving.getTileEntity(var12, var13, par5,
									true, false));
				}
